...
export interface PRExecution {
    ...
    title?: string;
    author?: string;
    status: 'pending' | 'analyzing' | 'completed' | 'failed' | 'deploying' | 'deployed' | 'deployment-blocked' | 'monitoring' | 'confirmed' | 'rolled-back';
    timestamp?: number;
...
export type SignalType = 'production.error_rate' | 'production.latency' | 'deployment.status' | 'ci.coverage' | 'ci.result' | 'time_of_day' | 'author_history' | 'simultaneous_deploy';
