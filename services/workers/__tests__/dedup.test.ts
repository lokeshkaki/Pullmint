import { deduplicateFindings, normalizedLevenshtein } from '../src/dedup';
...
describe('normalizedLevenshtein', () => {
  ...
  it('should return small distance for similar strings', () => {
    // "Missing null check" (18 chars) vs "Missing null check" (identical prefix)
    // edit distance = 10 chars added, maxLen = 28 → 10/28 ≈ 0.357
    // Use strings that are genuinely close (small edit distance relative to length)
  ...
  it('should return partial match distances correctly', () => {
    // "kitten" → "sitting" = distance 3, maxLen 7 → ~0.43
